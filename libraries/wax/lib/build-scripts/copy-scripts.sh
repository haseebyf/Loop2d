...
if [ $WAX_COMPILE_SCRIPTS ]; then
  ...
  lua "$PROJECT_DIR/wax/lib/build-scripts/luac.lua" wax wax.dat "$PROJECT_DIR/wax/lib/stdlib/" "$PROJECT_DIR/libraries/wax/lib/stdlib/init.lua" -L "$PROJECT_DIR/libraries/wax/lib/stdlib"/**/*.lua
  lua "$PROJECT_DIR/libraries/wax/lib/build-scripts/luac.lua" "" AppDelegate.dat "$SOURCE_SCRIPTS_DIR/" "$SOURCE_SCRIPTS_DIR/AppDelegate.lua" -L "$SOURCE_SCRIPTS_DIR"/**/*.lua
  ...
  cp -r "$PROJECT_DIR/libraries/wax/lib/stdlib" "$DESTINATION_SCRIPTS_DIR/wax"
